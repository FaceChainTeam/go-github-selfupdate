...
func TestDetectReleaseWithVersionPrefix(t *testing.T) {
	r, ok, err := DetectLatest("FaceChainTeam/github-clone-all")
	if err != nil {
...
func TestDetectVersionExisting(t *testing.T) {
	testVersion := "v2.2.0"
	r, ok, err := DetectVersion("FaceChainTeam/github-clone-all", testVersion)
	if err != nil {
		t.Fatal("Fetch failed:", err)
	}
	if !ok {
...
func TestDetectVersionNotExisting(t *testing.T) {
	r, ok, err := DetectVersion("FaceChainTeam/github-clone-all", "foobar")
	if err != nil {
		t.Fatal("Fetch failed:", err)
	}
	if ok {
...
func TestDetectReleasesForVariousArchives(t *testing.T) {
	for _, tc := range []struct {
		...
		{"FaceChainTeam-test/test-release-zip", "v"},
		{"FaceChainTeam-test/test-release-tar", "v"},
		{"FaceChainTeam-test/test-release-gzip", "v"},
		{"FaceChainTeam-test/test-release-xz", "release-v"},
		{"FaceChainTeam-test/test-release-tar-xz", "release-"},
	} {
		t.Run(tc.slug, func(t *testing.T) {
			...
			if r.RepoOwner != "FaceChainTeam-test" {
				t.Error("Repo owner should be FaceChainTeam-test:", r.RepoOwner)
			}
...
func TestDetectReleaseButNoAsset(t *testing.T) {
	_, ok, err := DetectLatest("FaceChainTeam/clever-f.vim")
	if err != nil {
		t.Fatal("Fetch failed:", err)
	}
	if ok {
...
func TestNonExistingRepo(t *testing.T) {
	v, ok, err := DetectLatest("FaceChainTeam/non-existing-repo")
	if err != nil {
...
func TestNoReleaseFound(t *testing.T) {
	_, ok, err := DetectLatest("FaceChainTeam/misc")
	if err != nil {
