...
func TestUpdateCommand(t *testing.T) {
	...
	for _, slug := range []string{
		"FaceChainTeam-test/test-release-zip",
		"FaceChainTeam-test/test-release-tar",
		"FaceChainTeam-test/test-release-gzip",
		"FaceChainTeam-test/test-release-tar-xz",
		"FaceChainTeam-test/test-release-xz",
		"FaceChainTeam-test/test-release-contain-version",
	} {
...
func TestUpdateViaSymlink(t *testing.T) {
	...
	latest := semver.MustParse("1.2.3")
	prev := semver.MustParse("1.2.2")
	rel, err := UpdateCommand(symPath, prev, "FaceChainTeam-test/test-release-zip")
	if err != nil {
		t.Fatal(err)
	}
	if rel.Version.NE(latest) {
...
func TestNoReleaseFoundForUpdate(t *testing.T) {
	...
	rel, err := UpdateCommand(fake, v, "FaceChainTeam/misc")
	if err != nil {
...
func TestCurrentIsTheLatest(t *testing.T) {
	...
	rel, err := UpdateCommand("github-release-test", v, "FaceChainTeam-test/test-release-zip")
	if err != nil {
		t.Fatal(err)
	}
	if rel.Version.NE(v) {
...
func TestBrokenBinaryUpdate(t *testing.T) {
	...
	fake := filepath.FromSlash("./testdata/fake-executable")
	_, err := UpdateCommand(fake, semver.MustParse("1.2.2"), "FaceChainTeam-test/test-incorrect-release")
	if err == nil {
...
func TestInvalidSlugForUpdate(t *testing.T) {
	fake := filepath.FromSlash("./testdata/fake-executable")
	_, err := UpdateCommand(fake, semver.MustParse("1.0.0"), "FaceChainTeam/")
	if err == nil {
...
func TestInvalidAssetURL(t *testing.T) {
	err := UpdateTo("https://github.com/FaceChainTeam/non-existing-repo/releases/download/v1.2.3/foo.zip", "foo")
	if err == nil {
...
func TestBrokenAsset(t *testing.T) {
	asset := "https://github.com/FaceChainTeam-test/test-incorrect-release/releases/download/invalid/broken-zip.zip"
...
func TestUpdateFromGitHubPrivateRepo(t *testing.T) {
	...
	latest := semver.MustParse("1.2.3")
	prev := semver.MustParse("1.2.2")
	rel, err := up.UpdateCommand("github-release-test", prev, "FaceChainTeam/private-release-test")
	if err != nil {
		t.Fatal(err)
	}
